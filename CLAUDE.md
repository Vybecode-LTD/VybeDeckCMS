...
**Phase 8–11 continuation: plugin settings UI, AI streaming, webhook events admin, album artwork crop** (commit `d87f379`):
- Plugin settings: `setting` DSL on `VybeDeck::Plugin::Base`; `Plugin#setting_value`/`#update_settings!`; `Admin::PluginSettingsController` (admin-only, nested under plugin); settings form at `/admin/plugins/:plugin_id/settings`; `SampleSeoPlugin` declares `inject_generator_meta` (boolean) and `generator_content` (string); `PluginPolicy#update?` added (was missing).
- AI streaming: `streaming boolean` on `ai_messages`; `StreamAiResponseJob` calls Anthropic SSE API, broadcasts deltas via `Turbo::StreamsChannel.broadcast_replace_to`, saves final content; `AiMessagesController` creates blank streaming placeholder and enqueues job; `turbo_stream_from` on conversation show; blinking cursor CSS.
- Webhook events admin: `StripeWebhookEvent` model + migration; webhook controller logs every event with idempotency guard; `Admin::StripeWebhookEventsController` (admin-only, index/show/replay); `ReplayStripeWebhookJob`; `StripeWebhookEventPolicy`; "Webhooks" nav link.
- Album artwork crop: `crop_x`/`crop_y` validations (columns pre-existed); `artwork_crop_controller.js` Stimulus controller; focal-point picker in album form; `object-position` on public album show.
- **Suite: 767 runs, 2021 assertions, 0 failures, 0 errors, 0 skips**
...
- Full Open Graph tags: `og:title/description/url/type/site_name/image`; Twitter Card `summary_large_image`
- JSON-LD: `content_for(:json_ld)` yielded as `<script type="application/ld+json">` in `<head>`
- Post show: sets `og:type "article"`, `og:image` from cover_image, Article JSON-LD with author/dates/image
- Page show: sets `og:image` from hero_image when attached
- `SitemapController`: `GET /sitemap.xml`; includes published posts/pages/albums/active products
- `RobotsController`: `GET /robots.txt` (text/plain, no layout); disallows admin/webhooks/account/checkout/cart; appends custom rules from `SiteSetting.get("robots_txt_custom")`
- Removed static `public/robots.txt` so dynamic route takes effect
- `SiteSetting` + admin settings UI: `robots_txt_custom` textarea
- **Suite: 713 runs, 1857 assertions, 0 failures, 0 errors, 0 skips**

**Phase 9 — Visual Design System Builder** (commit `fe0ccd6`):
- `Theme` model: JSON `tokens` column (12 colour tokens + font), `effective_tokens` merges defaults, `to_css` renders inline override block, `to_json_export`; `apply!`/`deactivate!`/`reset_to_defaults!` with Solid Cache bust
- `Theme.active_theme` class method (not scope) returns record or nil; `find_or_create_by!(name: "Default")` on first admin visit
- `Admin::ThemeController` (singular resource with `controller: :theme`): show, update, export, import, reset
- Live two-panel preview (light + dark) driven by Stimulus `theme_editor_controller.js`; colour picker ↔ text input sync; CSS vars injected live on preview panels; font select updates hidden `font_url` input and lazy-loads Google Font `<link>`
- 8 available fonts: Inter, Lato, Roboto, Montserrat, Playfair Display, Merriweather, Source Sans 3, Open Sans
- Active theme CSS injected as `<style id="theme-override">` via `active_theme_css` helper (Solid Cache, 1h TTL)
- "Design" nav link added for admin role; routes: `resource :theme, controller: :theme`

**Phase 8 — Plugin System** (commit `5d45984`):
- `VybeDeck::Plugin::Base` module + `VybeDeck::Plugin::Registry` singleton; `::Plugin` used to avoid constant resolution to `VybeDeck::Plugin` module
- `SampleSeoPlugin` in `lib/plugins/`; auto-loaded by `config/initializers/plugins.rb`
- `Plugin` AR model with status enum (installed/active/disabled), slug validation, lifecycle methods
- Admin `/admin/plugins` CRUD (admin-only); view hooks in public layout; model hooks in Post/Order via `after_commit`

**Phase 7 — Claude AI Assistant** (commit `f4ffd41`):
- `AiConversation` + `AiMessage` models; `AiAssistantService` uses `Net::HTTP` to Anthropic API; admin-only conversations
- Mock pattern: `define_method(:call)` + `instance_method` save/restore (Minitest 6, no stub)

**Phase 6 — Album Manager** (commit `777594a`):
- `Album`, `Track`, `AlbumCollaborator`, `TrackVersion`, `TrackComment` models (5 migrations)
- FriendlyId on Album (slugged + history); polymorphic `productable` on both Album and Track
- `Album#publish!`: validates artwork + release_date + audio track present; creates/reuses Product; admin-only via `AlbumPolicy#publish?`
- `Track#set_position` callback (`before_validation, on: :create`) — 0-based auto-increment (unconditional assignment avoids DB default 0 masking `||=`)
- `has_rich_text :lyrics` on Track; `has_one_attached :audio` + `has_one_attached :artwork` on Album
- Admin kanban at `/admin/albums` grouped by status; show with track list + native HTML5 DnD reorder
- `track_sort_controller.js` (Stimulus, no npm): DnD + `PATCH /admin/albums/:id/tracks/reorder`; `track_preview_controller.js` constrains audio to `[preview_start, preview_end]` seconds
- Album download report at `/admin/album_download_report` (paid `LineItem`s for Album/Track products); `skip_policy_scope` used instead of `policy_scope(LineItem)` since no `LineItemPolicy::Scope` exists
- Public `/albums` and `/albums/:slug` (`allow_unauthenticated_access`); show 404s on draft albums via `Album.published.friendly.find`
- **Suite: 713 runs, 1857 assertions, 0 failures, 0 errors, 0 skips**

**Phase 5 — Admin Group Chat** (commit `e8db0ce`):
...
Plugin settings, AI streaming, webhook admin, and album crop complete. Next: real content import (pages, posts, albums) via admin panel or `db/seeds/production.rb` before launch, then monitor first Railway deploy with smoke checklist.
...
```
767 runs, 2021 assertions, 0 failures, 0 errors, 0 skips
```
