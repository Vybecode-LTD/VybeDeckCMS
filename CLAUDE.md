...
Phase 4.1 (Forum models, admin, and public UI) complete and committed. Remaining Phase 4 sub-phases:
- **4.2 — Reactions & Moderation Queue**: `ForumReply` likes/thumbs; report flag → admin moderation queue; approve/remove per reply
- **4.3 — Per-Reply Admin Controls**: inline delete button in thread view (placeholder already in `_reply.html.erb`)
...
- `ForumReply` — `author` (User), `forum_thread`, Action Text `body`, `likes_count`, `is_solution`. `after_create_commit` updates thread `last_reply_at`; `after_destroy_commit` resets it (with cascade-destroy guard via `forum_thread.destroyed?`).
...
```
490 runs, 1292 assertions, 0 failures, 0 errors, 0 skips
```
