...
Phase 3.7 complete. Phase 3 E-Commerce (including all transactional emails) is done. Moving to **Phase 4 — Community & Forum**: `Forum`, `Thread`, `Reply` models; public `/community` routes with visibility gating (public / members-only / subscriber-only); Turbo Stream reply posting; reactions and moderation queue; notification bell in site header. See ROADMAP.md Phase 4.

**Pre-requisite before Phase 4 ships to production:** Set `SMTP_ADDRESS`, `SMTP_USERNAME`, `SMTP_PASSWORD` as Railway env vars. All transactional email code (order confirmation, download-ready, refund receipt, email verification, password reset) is complete — SMTP is the only remaining blocker for live delivery.
...
```
444 runs, 1110 assertions, 0 failures, 0 errors, 0 skips
```
