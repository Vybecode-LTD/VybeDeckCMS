# Solid Queue tables.
#
# This migration is idempotent (if_not_exists: true / foreign_key_exists? guards)
# so it is safe to run against a database that already has some or all of these
# tables (e.g. created by an earlier db:schema:load:queue run).
#
# Matches db/queue_schema.rb version 1 (solid_queue 1.4.0).

class InstallSolidQueue < ActiveRecord::Migration[8.0]
  def up
    # Parent table — must be created before child tables that FK to it.
